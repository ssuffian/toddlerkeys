...
export class SessionController {
  ...
  constructor(private changed: (snapshot: AppSnapshot) => void = () => {}, settings: Settings = { sound: true, volume: 0.15, reducedMotion: false, lockdownMode: false }) { this.settings = { ...settings }; }
  ...
  updateSettings(value: unknown) {
    ...
    if (typeof s.sound !== 'boolean' || typeof s.reducedMotion !== 'boolean' || typeof s.lockdownMode !== 'boolean' || typeof s.volume !== 'number' || !Number.isFinite(s.volume) || Object.keys(s).some(k => !['sound', 'volume', 'reducedMotion', 'lockdownMode'].includes(k))) return false;
    this.settings = { sound: s.sound, volume: Math.max(0, Math.min(1, s.volume)), reducedMotion: s.reducedMotion, lockdownMode: s.lockdownMode }; this.publish(); return true;
  }
}
