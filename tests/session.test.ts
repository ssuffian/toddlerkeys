...
describe('session lifecycle', () => {
  ...
  it('validates, clamps, and locks settings while playing', () => {
    const controller = new SessionController();
    expect(controller.updateSettings({ sound: false, volume: 2, reducedMotion: true, lockdownMode: false })).toBe(true);
    expect(controller.snapshot().settings).toEqual({ sound: false, volume: 1, reducedMotion: true, lockdownMode: false });
    expect(controller.updateSettings({ sound: true, volume: Number.NaN, reducedMotion: false, lockdownMode: false })).toBe(false);
    practice(controller);
    controller.start();
    expect(controller.updateSettings({ sound: true, volume: 0.5, reducedMotion: false, lockdownMode: false })).toBe(false);
  });
